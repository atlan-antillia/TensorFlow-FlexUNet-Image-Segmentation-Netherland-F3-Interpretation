...
class ImageMaskDatasetGenerator:
  ...
  def generate(self, images_dir, masks_dir, output_images_dir, output_masks_dir):
      ...
      if sn != fn:
        raise Exception("Unmached number of seismic and fault")
      num = sn
      index = 1001
      for n in range(num):
        
        image = cv2.imread(image_files[n], cv2.IMREAD_UNCHANGED)
        mask  = cv2.imread(mask_files[n])
        mask  = cv2.cvtColor(mask, cv2.COLOR_BGR2GRAY)  
        
        image = self.crop_resize(image)
        mask  = self.crop_resize(mask)

        filename = str(index + n) + "_" + ".png"
        output_imagefile = os.path.join(output_images_dir, filename)
        output_maskfile  = os.path.join(output_masks_dir, filename)
   
        colorized = self.colorize_mask(mask)

        image     = np.expand_dims(image, axis=-1)
     
        cv2.imwrite(output_imagefile, image)
        cv2.imwrite(output_maskfile,  colorized)
        
        print("Saved {}".format(output_imagefile))
        print("Saved {}".format(output_maskfile))
        if self.augmentation:
...
if __name__ == "__main__":
  try:
    images_dir = "./Netherland-F3/crosslines/"
    masks_dir  = "./Netherland-F3/masks/"

    output_dir = "./Augmented-Netherland-F3-master/"
    ...
    augmentation = True
    generator =  ImageMaskDatasetGenerator(augmentation=augmentation)
